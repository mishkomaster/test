...
const generateMovieRow = (movie: Movie) => {
    ...
    oscarsCell.classList.add("movie-data", "movie-oscars");
     ...
     oscarsCell.addEventListener("click", () => {
      ...
      const modalHeader = document.querySelector("#oscars-details-content h2")! as HTMLHeadingElement;
      modalHeader.innerText = `Selected Oscars for ${movie.movie_name}`; // Use backticks for the template literal
  
      const oscarsList = document.getElementById("movie-details-oscar-list")! as HTMLUListElement; // Update the ID to "movie-details-oscar-list"
